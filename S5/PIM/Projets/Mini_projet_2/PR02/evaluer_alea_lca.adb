...
procedure Evaluer_Alea_LCA is
   ...
   begin
       ...
       for i in 1..Taille loop
           Get_Random_Number(Nombre_A);
           if not(Cle_Presente(L,Nombre_A)) then
               Enregistrer(L, Nombre_A, 1);
           else
               Enregistrer(L, Nombre_A, La_Donnee(L, Nombre_A)+1);
           end if;
       end loop;

       Min:=La_Donnee(L,Nombre_A);
       Max:=La_Donnee(L,Nombre_A);
