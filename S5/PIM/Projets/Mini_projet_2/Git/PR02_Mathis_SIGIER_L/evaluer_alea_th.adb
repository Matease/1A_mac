...
procedure Evaluer_Alea_TH is
    ...
    function Hachage_identite_TH (i : integer) return Integer is
    begin
        return(i);
    end Hachage_identite_TH;
	...
	is
        ...
        package TH_alea is new TH (T_Cle => Integer, T_Donnee => Integer, Capacite => Borne, Hachage => Hachage_identite_TH);
        use TH_alea;
	...
	begin
        ...
        for i in 1..Taille loop
            ...
            if not(Cle_Presente(L, Nombre_alea)) then
                Enregistrer(L, Nombre_alea, 1);
            else
                Enregistrer(L, Nombre_alea, La_Donnee(L, Nombre_alea)+1);
            end if;
        end loop;
        
        Min:=La_Donnee(L,Nombre_alea);
        Max:=La_Donnee(L,Nombre_alea);

        for i in 1..Taille loop
            if not Cle_Presente(L,i) then
                Null;
            elsif Min >  La_Donnee(L,i) then
                Min:=La_Donnee(L,i);
            elsif Max < La_Donnee(L,i) then
                Max:=La_Donnee(L,i);
            end if;
        end loop;
