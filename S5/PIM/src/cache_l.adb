...
package body Cache_L is
   ...
   procedure Initialiser(Cache : out T_Cache_L) is
   begin
      Cache := Null;
   end Initialiser;


   -- fonction servant a savoir si le cache est plein avec Taille_Cache representant la taile maximale du cache fixée par l'utilisateur (constante tout au long de l'utilisation du routeur).
   function Cache_plein (Cache : in T_Cache_L; Taille_Cache : in Integer ) return Boolean is
   ...
   -- procedure servant a supprimer une route du cache.
  ...
  procedure Ajouter_Route_Debut (Cache : in out T_Cache_L; Route : in T_Route) is
   ...
   procedure Ajouter_Route_Fin (Cache : in out T_Cache_L; Route : in T_Route) is
    ...
    begin
        if Cache /= null then
            Ajouter_Route_Fin (Cache.Suivant, Route);
        else
   ...
   function Chercher (Cache : in T_Cache_L; adresse_a_comparer : in T_Adresse_IP) return T_Route is
    ...
    begin
     ...
     procedure Enregistrer (Cache : in out T_Cache_L; Route : in T_Route ; politique : in String; Taille_Cache : in Integer) is
    begin
        if Cache_plein(Cache,Taille_Cache) then
            Supprimer_Derniere_Route(Cache);
        end if;
   ...
   procedure Afficher_Cache(Cache : in out T_Cache_L) is
        Cache2 : T_Cache_L := Cache;
    begin
        Put_Line("Cache");
        while Cache2 /= null loop
            ...
            Put(" ");
            Put(Cache2.all.Route.Interface_Associee);
            New_Line;
            Cache2 := Cache2.Suivant;
        end loop;
        New_Line;
