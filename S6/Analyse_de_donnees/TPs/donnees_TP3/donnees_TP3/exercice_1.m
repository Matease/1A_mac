...
% function V = vraisemblance(val1, val2, mu, sigma)
%     x = [val1 val2];
%     xm = x - mu;
%     invsigma = inv(sigma);
%     V = (1/(2*pi*(det(sigma))0.5))*exp(-0.5.xm'*invsigma*xm);
% end
...
%fonction estimaitojn mu sigma
function [mu2, sigma] = estimation_mu_Sigma(X)
    d = size(X,1);
    mu = mean(X);
    mu2 = mu;
    sigma = (X-mu)'*(X-mu)/d;
...
function V = vraisemblance(val1, val2, mu, sigma)
    ...
    xm = x - mu';
    %invsigma = inv(sigma);
    V = (1/(2*pi*sqrt(det(sigma))))*exp(-0.5*((xm)')*(sigma\xm));
end
