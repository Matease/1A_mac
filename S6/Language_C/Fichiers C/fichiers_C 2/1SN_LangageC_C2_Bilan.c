...
void create(string *chaine_dest, char *chaine_src){
    int N = strlen(chaine_src) + 1;
    chaine_dest->str = (char *) malloc(N * sizeof(char));
...
void add(string *chaine, char c){
    int N = chaine->N + 1;
    chaine->str = (char *) realloc(chaine->str, N * sizeof(char));
...
void delete(string *chaine, int i){
    assert (i > 0 || i <= chaine->N - 1);
    for (int j = i + 1; j < chaine->N; j++) {
        chaine->str[j - 1] = chaine->str[j];
    }
    chaine->N--;
    chaine->str = (char *) realloc(chaine->str, chaine->N * sizeof(char));
    assert (chaine->str != NULL && chaine->N >= 0);
}
